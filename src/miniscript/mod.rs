...
impl<Ctx: ScriptContext> Miniscript<Ctx::Key, Ctx> {
    ...
    /// use miniscript::bitcoin::{hex, XOnlyPublicKey};
    /// use bitcoin::script::{TapScriptBuf, WitnessScriptBuf};
    ///
    /// type Segwitv0Script = Miniscript<bitcoin::PublicKey, Segwitv0>;
    /// type TapScript = Miniscript<XOnlyPublicKey, Tap>;
    ///
    /// // parse x-only miniscript in Taproot context
    /// let tapscript_ms = TapScript::decode(&TapScriptBuf::from(
    ///     hex::decode_to_vec("202788ee41e76f4f3af603da5bc8fa22997bc0344bb0f95666ba6aaff0242baa99ac")
    ...
    /// let err = TapScript::decode(&TapScriptBuf::from(
    ///     hex::decode_to_vec("21022788ee41e76f4f3af603da5bc8fa22997bc0344bb0f95666ba6aaff0242baa99ac")
    ///         .expect("Even length hex")
    /// ))
    ///     .expect_err("Compressed keys cannot be used in Taproot context");
    /// // Segwitv0 succeeds decoding with full keys.
    /// Segwitv0Script::decode(&WitnessScriptBuf::from(
    ///     hex::decode_to_vec("21022788ee41e76f4f3af603da5bc8fa22997bc0344bb0f95666ba6aaff0242baa99ac")
    ///         .expect("Even length hex")
    /// ))
...
pub mod hash256 {
    ...
    impl core::str::FromStr for Hash {
        type Err = bitcoin::hex::DecodeFixedLengthBytesError;
...
mod tests {
    ...
    fn all_attribute_tests() {
        ...
        ms_attributes_test("and_b(hash256(32ba476771d01e37807990ead8719f08af494723de1d228f2c2c07cc0aa40bac),a:and_b(hash256(131772552c01444cd81360818376a040b7c3b2b7b0a53550ee3edde216cec61b),a:older(1)))", "82012088aa20ac0ba40acc072c2c8f221dde234749af089f71d8ea907980371ed0716747ba32876b82012088aa201bc6ce16e2dd3eee5035a5b0b7b2c3b740a07683816013d84c44012c55721713876b51b26c9a6c9a", true, true, false, 15, 2);
    ...
    fn serialize() {
        ...
        roundtrip(
            ...
            "\
             OP_IF OP_SIZE OP_PUSHBYTES_1 20 OP_EQUALVERIFY OP_HASH256 OP_PUSHBYTES_32 1bc6ce16e2dd3eee5035a5b0b7b2c3b740a07683816013d84c44012c55721713 OP_EQUAL \
    ...
    fn multi_a_tests() {
        ...
        // script rtt test
        //
        // Claud wrote: Note: decode_consensus not available for bitcoin::key::XOnlyPublicKey
        // 
        // FIXME: We should be able to un-comment this, I _think_ we can't because of
        //
        // **Taken from** ./context.rs
        // impl ScriptContext for Tap {
        // FIXME: Does this have to change to `bitcoin::XOnlyPublicKey`.
    ...
    fn expr_features() {
        ...
        .unwrap();
        let hash160 = hash160::Hash::from_byte_array(pk.pubkey_hash().to_byte_array());
    ...
    fn test_script_parse_dos() {
        let mut script: bitcoin::script::Builder<script::TapScriptBuf> = bitcoin::script::Builder::new().push_opcode(bitcoin::opcodes::all::OP_1);
        for _ in 0..10000 {
