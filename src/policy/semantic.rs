...
impl fmt::Display for MathSyntaxError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            MathSyntaxError::SingletonGroup => {
                f.write_str("singleton group; expected `∧` or `∨` between operands")
            }
            MathSyntaxError::MixedOperators => f.write_str("mixed `∧`/`∨` in same group"),
            MathSyntaxError::UnknownTerminal(name) => write!(f, "unknown terminal `{}`", name),
            MathSyntaxError::Other(s) => f.write_str(s),
        }
    }
}
